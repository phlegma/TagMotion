...
using Chrismo.TagMotion;

namespace Chrismo.Mp3
{
	
	
	public class SaveFileDialogWithSortType
	{
		...
		private int HookProc(int hdlg, int msg, int wParam, int lParam)
		{
			switch (msg)
			{
				case WM_INITDIALOG:
					...
					SetWindowText(labelHandle, "&Sorting:");
					...
					//and add the sorttypes we want to offer
					SendMessage(comboHandle, CB_ADDSTRING, 0, "By Year");
					SendMessage(comboHandle, CB_ADDSTRING, 0, "By Artist");
				...
				case WM_NOTIFY:
					...
					if (nmhdr.Code==CDN_FILEOK)
					{
						...
						//we need to get the sorttype

						m_SortType = (SortType)SendMessage(m_ComboHandle, CB_GETCURSEL, 0, 0);
					}
					break;
